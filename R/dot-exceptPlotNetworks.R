.exceptPlotNetworks<- function(args) {
  ...
  # x
  stopifnot(inherits(args$x, "microNetProps"))
  ...
  if (!is.null(args$labelPattern)) {
    stopifnot(is.vector(args$labelPattern) & 
  ...
  # nodeFilter
  args$nodeFilter <- match.arg(args$nodeFilter, 
                               choices = c("none", "highestConnect",
