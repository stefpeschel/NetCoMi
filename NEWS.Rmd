...
* In `netConstruct()`: Using **"bootstrap"** as sparsification method in 
...
for network construction, but have no row and column names.
...
* `plot.microNetProps()` now directly returns an error if `x` has not the 
expected class.
